...
const applyFilters = () => {
  router.get('/tenant/report-cards', {
    class_id: classId.value, semester_id: semesterId.value,
  }, { preserveState: true });
};
...
const generateReportCards = () => {
  ...
  router.post('/tenant/report-cards/generate', {
    class_id: classId.value, semester_id: semesterId.value,
            ...
            <tr v-for="rc in reportCards.data" :key="rc.id" class="border-b border-slate-50 transition-colors hover:bg-slate-50/50">
              ...
              <td class="px-4 py-3">
                <div class="flex items-center gap-3">
                  ...
                  <span class="font-medium text-slate-700">{{ rc.student?.name }}</span>
                </div>
              </td>
              ...
              <td class="px-4 py-3">
                <div class="flex items-center justify-end gap-1">
                  <Button :href="`/tenant/report-cards/${rc.id}`" variant="ghost" size="xs">Detail</Button>
                  <Link
                    v-if="rc.status === 'draft'"
                    :href="`/tenant/report-cards/${rc.id}/publish`"
                    method="post"
                    as="button"
                  ...
                  <a :href="`/tenant/report-cards/${rc.id}/pdf`" class="rounded-lg px-2.5 py-1 text-xs font-medium text-slate-500 transition-colors hover:bg-slate-100">
                    PDF
                  </a>
                </div>
              </td>
