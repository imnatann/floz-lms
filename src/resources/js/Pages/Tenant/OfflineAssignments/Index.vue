...
const applyFilters = debounce(() => {
  router.get('/tenant/offline-assignments', {
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
      <div>
        ...
        <nav class="flex text-sm text-slate-500 mt-1">
          <Link href="/tenant/dashboard" class="hover:text-blue-600">Home</Link>
          <span class="mx-2">/</span>
          <span class="text-slate-800 font-medium">Tugas Offline</span>
        </nav>
      </div>
      <Link v-if="!is_student" href="/tenant/offline-assignments/create">
          ...
          <tbody class="divide-y divide-slate-200">
            ...
            <tr v-for="assignment in assignments.data" :key="assignment.id" class="hover:bg-slate-50 transition-all">
              ...
              <td class="px-6 py-4 text-right flex justify-end gap-2">
                <Link :href="`/tenant/offline-assignments/${assignment.id}`" class="p-2 text-blue-600 hover:bg-blue-50 rounded-lg transition-colors" title="Detail">
                  <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"></path></svg>
                </Link>
                <template v-if="!is_student">
                    <Link :href="`/tenant/offline-assignments/${assignment.id}/edit`" class="p-2 text-amber-500 hover:bg-amber-50 rounded-lg transition-colors" title="Edit">
