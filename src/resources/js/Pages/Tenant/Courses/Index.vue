  ...
  <div class="space-y-6">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
      <Link
        ...
        :key="course.id"
        :href="`/tenant/courses/${course.id}`"
        class="group bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden hover:shadow-md hover:border-blue-200 transition-all duration-300"
      >
        <!-- Color Banner -->
        <div class="h-2.5 bg-gradient-to-r" :class="[
          course.subject?.name?.includes('Matematika') ? 'from-blue-500 to-indigo-500' :
          course.subject?.name?.includes('Bahasa') ? 'from-emerald-500 to-teal-500' :
          course.subject?.name?.includes('IPA') || course.subject?.name?.includes('Fisika') ? 'from-purple-500 to-violet-500' :
          course.subject?.name?.includes('IPS') || course.subject?.name?.includes('Sejarah') ? 'from-amber-500 to-orange-500' :
        ...
        <div class="p-5">
          ...
          <div class="mt-4 flex justify-end">
            <div class="w-8 h-8 rounded-full bg-slate-50 group-hover:bg-blue-50 flex items-center justify-center transition-colors">
              <svg class="w-4 h-4 text-slate-400 group-hover:text-blue-500 transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"></path></svg>
            </div>
          </div>
        </div>
      </Link>
