...
const submit = () => {
  if (isEdit) {
    ...
    form.post('/tenant/classes', { preserveScroll: true });
  }
};
</script>
...
<template>
  ...
  <div class="max-w-2xl mx-auto">
    <div class="mb-6 flex items-center justify-between">
      ...
      <Link href="/tenant/classes" class="inline-flex items-center gap-1.5 rounded-lg border border-slate-200 bg-white px-3.5 py-2 text-xs font-medium text-slate-600 shadow-sm hover:bg-slate-50">
    ...
    <form @submit.prevent="submit" class="rounded-xl border border-slate-200 bg-white shadow-sm">
      ...
      <div class="flex items-center justify-end gap-3 border-t border-slate-100 bg-slate-50/50 px-6 py-4 rounded-b-xl">
        <Link href="/tenant/classes" class="rounded-lg border border-slate-200 bg-white px-4 py-2 text-sm font-medium text-slate-600 shadow-sm hover:bg-slate-50">Batal</Link>
