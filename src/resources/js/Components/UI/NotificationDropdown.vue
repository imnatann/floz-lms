...
const fetchNotifications = async () => {
    loading.value = true;
    try {
        const response = await axios.get('/tenant/notifications/data');
...
onMounted(() => {
    if (user.value) {
        ...
        window.Echo.private(`App.Models.Tenant.User.${user.value.id}`)
...
<template>
    <div class="relative notification-dropdown">
        ...
        <button 
            ...
            class="relative rounded-full p-2 text-slate-400 hover:bg-slate-100 hover:text-slate-600 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:ring-offset-2"
        >
        ...
        >
            ...
            <div class="p-2 border-t border-slate-100 bg-slate-50/50 rounded-b-xl text-center">
                 <Link href="/tenant/notifications" class="text-xs font-medium text-slate-500 hover:text-slate-800 block w-full py-1">
