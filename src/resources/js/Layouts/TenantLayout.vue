...
const navigation = computed(() => {
  ...
  const items = [
    { name: 'Dashboard', href: '/tenant/dashboard', icon: 'dashboard', show: true },
    { name: 'Kelas', href: '/tenant/classes', icon: 'classes', show: permissions.manage_classes },
    { name: 'Mata Pelajaran', href: '/tenant/subjects', icon: 'subjects', show: permissions.manage_subjects },
    { name: 'Siswa', href: '/tenant/students', icon: 'students', show: permissions.view_all_students },
    { name: 'Guru & Staff', href: '/tenant/staff', icon: 'staff', show: permissions.manage_teachers },
    { name: 'Penugasan', href: '/tenant/teaching-assignments', icon: 'assignments', show: permissions.manage_assignments },
    { name: 'Kursus', href: '/tenant/courses', icon: 'subjects', show: true },
    { name: 'Tugas', href: '/tenant/assignments', icon: 'homework', show: true },
    { name: 'Nilai', href: '/tenant/grades', icon: 'grades', show: permissions.manage_grades }, // Only Teachers/Admins can input grades
    { name: 'Jadwal', href: '/tenant/schedules', icon: 'schedule', show: true },
    { name: 'Rapor', href: '/tenant/report-cards', icon: 'reports', show: true }, // Everyone needs raports/transcripts
    { name: 'Absensi', href: '/tenant/attendance', icon: 'attendance', show: !isStudent.value }, // Hide from students (they see summary in Dashboard)
    { name: 'Pengumuman', href: '/tenant/announcements', icon: 'announcements', show: true },
    { name: 'Audit Logs', href: '/tenant/audit-logs', icon: 'audit-logs', show: user.value?.role === 'school_admin' },
  ];
          ...
          <div class="relative">
            ...
            >
              <div v-if="showUserMenu" class="absolute right-0 mt-2 w-48 origin-top-right rounded-xl border border-slate-200 bg-white p-1.5 shadow-lg shadow-slate-200/50">
                ...
                <Link
                  v-if="user?.student"
                  :href="`/tenant/students/${user.student.id}`"
